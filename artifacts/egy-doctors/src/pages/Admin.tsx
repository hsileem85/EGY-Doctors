...
function FinancialManagementSection({ lang }: { lang: "en" | "ar" }) {
  ...
  const [form, setForm] = useState<AdminFinancialSettings>({ deductionType: "PERCENTAGE", deductionValue: 0, platformSharePercentage: 100, cashbackSharePercentage: 0, minDoctorWalletBalance: 0, subscriptionModelEnabled: false, reviewPromptDelayHours: 6, reviewCashbackAmount: 25 });
  ...
  return <div className="space-y-6 max-w-4xl">
    ...
    {subTab === "settings" ? <div className="space-y-5">
      <div className="bg-white border rounded-xl p-6"><h3 className="font-semibold mb-4">{isAr ? "الخصم وحصص الإيرادات" : "Deduction & revenue shares"}</h3>{settings.isLoading ? <p className="text-gray-500">Loading...</p> : settings.isError ? <p className="text-red-600">{isAr ? "تعذر تحميل الإعدادات" : "Unable to load settings"}</p> : <div className="grid sm:grid-cols-2 gap-4">
        {numberField(isAr ? "قيمة الخصم" : "Deduction value", "deductionValue")}{numberField(isAr ? "حصة المنصة %" : "Platform share %", "platformSharePercentage")}{numberField(isAr ? "حصة الكاش باك %" : "Cashback share %", "cashbackSharePercentage")}
        <div className="sm:col-span-2 border-t pt-4"><h4 className="font-semibold text-gray-800">{isAr ? "مكافأة تقييم المريض" : "Patient review reward"}</h4><p className="text-xs text-gray-500 mt-1 mb-3">{isAr ? "تحدد هذه الإعدادات وقت ظهور طلب التقييم وقيمة الكاش باك بعد اكتمال الموعد." : "Controls when the post-completion review prompt appears and its cashback reward."}</p><div className="grid sm:grid-cols-2 gap-4">{numberField(isAr ? "تأخير طلب التقييم (ساعات)" : "Review prompt delay (hours)", "reviewPromptDelayHours")}{numberField(isAr ? "كاش باك التقييم (جنيه)" : "Review cashback (EGP)", "reviewCashbackAmount")}</div></div>
      </div>}<p className={`text-xs mt-3 ${valid ? "text-gray-500" : "text-red-600"}`}>{isAr ? "يجب أن يكون مجموع حصة المنصة والكاش باك 100%." : "Platform and cashback shares must total 100%."}</p><div className="mt-5"><Button disabled={!valid || save.isPending} onClick={() => save.mutate({ ...form, subscriptionModelEnabled: false })} className="bg-[#D4A853] text-[#0F172A]"><Save className="w-4 h-4 mr-2" />{isAr ? "حفظ" : "Save settings"}</Button>{save.isSuccess && <span className="text-sm text-green-600 ml-3">{isAr ? "تم الحفظ" : "Saved"}</span>}{save.isError && <span className="text-sm text-red-600 ml-3">{isAr ? "فشل الحفظ" : "Save failed"}</span>}</div></div>
