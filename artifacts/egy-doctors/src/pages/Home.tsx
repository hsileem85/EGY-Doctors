          ...
          {isLoading ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "0.75rem" }}>
              {sortedDoctors.map((doc) => (
                <div
                  key={doc.id}
                  className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:border-slate-200 hover:-translate-y-0.5 transition-all duration-200 w-full p-3"
                ...
                >
                  ...
                  {/* Details: name + specialty + price + location — fills all central space */}
                  <div className="flex flex-col gap-1 min-w-0 flex-1 pl-1">
                    <Link href={`/doctor/${doc.id}`}>
                      <h3 className="text-[13px] font-bold text-slate-900 leading-tight hover:text-[#D4A853] cursor-pointer transition-colors truncate">
                        {doc.name}
                      </h3>
                    </Link>
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-1.5">
                    ...
                    <Link href={`/profile/${doc.id}`}>
                      <Button
                        variant="outline"
                        className="text-slate-700 rounded-xl font-semibold text-[10px] border-slate-200 hover:bg-slate-50 hover:border-slate-300 transition-all h-7 px-3 whitespace-nowrap"
                      >
                        {isRTL ? "الملف" : "Profile"}
                      </Button>
                    </Link>
