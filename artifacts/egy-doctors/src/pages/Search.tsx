...
export default function Search() {
  ...
  const filteredDoctors = useMemo(() => {
    return doctors.filter(doctor => {
      ...
      const matchLocation = selectedLocations.length === 0 || selectedLocations.includes(doctor.location);
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="relative max-w-2xl mx-auto">
            ...
            <Input 
              ...
              className="pl-12 h-12 rounded-full bg-gray-50 border-transparent focus:bg-white focus:border-primary focus:ring-primary shadow-sm"
              value={searchQuery}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-xl border p-6 sticky top-40 shadow-sm">
              ...
              <div className="mb-8">
                <h3 className="text-sm font-semibold text-gray-900 mb-4 uppercase tracking-wider">Specialty</h3>
                <div className="space-y-3">
                  {specialties.map(s => (
                    <div key={s} className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor={`spec-${s}`} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">
                        {s}
                      </Label>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {locations.map(l => (
                    <div key={l} className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor={`loc-${l}`} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">
                        {l}
                      </Label>
                    </div>
                  ))}
