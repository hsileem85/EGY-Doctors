      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button className="text-sm font-medium text-[#0F172A] px-4 py-2 hover:bg-gray-50 rounded-lg">Sign In</button>
            <button className="text-sm font-medium text-white px-4 py-2 bg-[#0F172A] rounded-lg">For Doctors</button>
          </div>
        </div>
      </nav>
      ...
      <div className="flex-1 pt-24 pb-32 max-w-3xl mx-auto w-full px-4">
        <div className="space-y-8">
          ...
          <div className="flex flex-col items-start max-w-xl">
             <div className="w-10 h-10 rounded-full bg-[#D4A853] flex items-center justify-center mb-3 shadow-md">
                <svg className="w-5 h-5 text-[#0F172A]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                   <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
             </div>
             <div className="bg-white p-5 rounded-2xl rounded-tl-sm shadow-sm border border-gray-100 text-[#0F172A]">
             ...
             {step === 0 && (
               <div className="mt-4 flex flex-wrap gap-2">
                 ...
                 <button className="px-4 py-2 bg-[#0F172A] border border-[#0F172A] rounded-full text-sm font-medium text-white shadow-sm">
                   Type specialty...
                 </button>
               </div>
             )}
          ...
          {step > 0 && (
            <div className="flex flex-col items-end w-full">
              <div className="bg-[#0F172A] text-white p-4 rounded-2xl rounded-tr-sm shadow-sm max-w-sm">
          ...
          {step > 2 && (
            <div className="flex flex-col items-start w-full animate-in slide-in-from-bottom-2 fade-in duration-300">
               ...
               <div className="bg-white p-5 rounded-2xl rounded-tl-sm shadow-sm border border-gray-100 text-[#0F172A] w-full mb-6">
                     ...
                     <div key={doctor.id} className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm hover:border-[#D4A853]/50 hover:shadow-md transition-all">
                       <div className="flex gap-4">
                         ...
                         <div className="flex-1">
                           <h3 className="font-bold text-[#0F172A] leading-tight">{doctor.name}</h3>
                           <p className="text-sm text-[#D4A853] font-medium mb-1">{doctor.specialty}</p>
                       ...
                       <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-2 text-xs text-gray-500 mb-4">
                         ...
                         <span className="bg-gray-50 px-2 py-1 rounded-md text-[#0F172A] font-semibold">{doctor.fee} EGP</span>
                       </div>

                       <button className="w-full py-2.5 bg-[#0F172A] text-white rounded-xl text-sm font-medium hover:bg-[#1E293B] transition-colors">
                         Book Appointment
                       </button>
                     </div>
                  ))}
               ...
               <div className="mt-8 flex justify-center w-full">
                 <button onClick={() => {setStep(0); setSelectedSpecialty(null); setSelectedLocation(null); setSelectedTime(null);}} className="text-sm text-gray-500 hover:text-[#0F172A] underline underline-offset-4">
                   Start a new search
                 </button>
               </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
         <div className="max-w-3xl mx-auto relative">
           <input 
             ...
             disabled={step < 3}
           />
           <button className="absolute right-2 top-2 p-2 bg-[#0F172A] rounded-full text-white hover:bg-[#1E293B] transition-colors disabled:opacity-50" disabled={step < 3}>
